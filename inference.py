...
from utils.checkpoints import create_checkpoint_filename, get_file_checkpoints
...
def main():
    ...
    epochs_ckpts = get_file_checkpoints(args) # Dict kind where key : epoch and value : .pt file path
    ...
    if args.dataset.lower() in ['housing', 'breast_cancer', 'digits', 'cifar10']:
        ...
        X = X.to(device)
        y = y.to(device)
        train_loader, test_loader, val_loader = create_dataloaders(X, y, batch_size=args.batch, test_frac=0.1,
                                                                   val_frac=0.1, device=device)
    ...
    if args.dataset.lower() == 'digits':
        ...
        if args.model.lower() == 'multi_class':
            ...
            import model_run.inference.multi_class_classification as main_modules
